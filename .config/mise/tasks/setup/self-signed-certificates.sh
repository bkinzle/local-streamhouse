...
then
  ...
  cp ${MISE_PROJECT_ROOT}/.ssl/root-ca.pem $(brew --prefix)/etc/ca-certificates/${DNSMASQ_DOMAIN}/local-oy11y-stack-ca.crt
  echo "Enter your mac's password and verify trusting to add this to keychain..."
  sudo security add-trusted-cert -d -r trustRoot -k /Library/Keychains/System.keychain $(brew --prefix)/etc/ca-certificates/${DNSMASQ_DOMAIN}/local-oy11y-stack-ca.crt
